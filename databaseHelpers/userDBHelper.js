 ...
 */
function registerUserInDB(username, password, registrationCallback, role = 1, companyId = 1){

  //const registerUserQuery = `INSERT INTO users (user_id, username, user_password) VALUES ('${username}', '${username}', SHA('${password}'))`
  const registerUserQuery = `CALL createUserAndRoles('${username}', '${password}', ${role}, ${companyId})`
  //execute the query to register the user
  mySqlConnection.query(registerUserQuery, registrationCallback)

}

function getCompanyIdFromKeyAndSecret(companyKey, companySecret, registrationCallback){
  
    //create query that includes the company_id
    const registerUserQuery = `INSERT INTO users (user_id, username, user_password, company_id) VALUES ('${username}', '${username}', SHA('${password}', ${companyId}))`
...
function configureClientAsUserInDB(username, password, companyId, registrationCallback){
