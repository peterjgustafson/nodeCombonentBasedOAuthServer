//Based on IBMS front end demo example. I have modified this heavily to force found keyword results into the results transcript.
...
//specifically replace the words two and for with numbers for now
          utterance.timestamps.forEach(function(word, index, theArray){
            

              if(word[0] == "to") {
                //console.log(word[0]);
                theArray[index][0] = "two";
                //word[0] == "two";
              }

              if(word[0] == "for") {
                //console.log(word[0]);
                theArray[index][0] = "four";
                word[0] == "four";
              }

              //console.log(word[theArray[index][0]]);
...
// overwrite the keywords spotted
    ...
    //console.log("UTTER:", utterance.timestamps);
    //console.log("UTTER Start:", utterance.timestamps[0][1]);
    //console.log("UTTER End:", utterance.timestamps[0][2]);
    ...
    if (original_keywords_result) {
      ...
      Object.keys(original_keywords_result).forEach(function(keyword) {
        ...
        if (spottings.length) {
          // console.log("spottings:", spottings);
          //if we can match the timespamps, now let's make sure we 
          //replcace all that have matching keywords with the actual keywords
          var uttenerancesToDelete = [];
          utterance.timestamps.forEach(function(word, index, theArray){
            // console.log("word:", word[0]);
            // console.log("start:", word[1]);
            // console.log("end", word[2]);
            spottings.forEach(function(spotting, index2, theArray2){
              if(spotting.start_time == word[1] && spotting.end_time == word[2]) {
                // console.log("word found");
                theArray[index][0] = spotting.normalized_text;
              }
              else if (spotting.start_time == word[1]) {
                if(index2 + 1 < theArray2.length && theArray2[index2+1].end_time == word[2]) {
                  // console.log("two words found");
                  //since we are matching one keyword with two words, we need to delete the
                  //extraneous word from the transcript so the timestamps still line up.
                  //since we are looping over this array, we just need a lint of indexs to delete after the loop.
                  uttenerancesToDelete.push(index2+1); 
                }
              }
            });
          ...
          //delete the extra utterances
          uttenerancesToDelete.forEach(function(index) {
            utterance.timestamps.splice(index, 1);
          });
    ...
    //one last odification is to change the chantities to numbers
          utterance.timestamps.forEach(function(word, index, theArray, convertNumbers = true){
